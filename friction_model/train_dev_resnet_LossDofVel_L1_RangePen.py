...
def train(model, train_loader, val_loader, criterion, optimizer, scheduler, device, session_save_dir, num_epochs, patience):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            loss_bound = torch.mean(boundary_violations**2)
            loss = loss_cri + loss_bound
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                loss_bound_val = torch.mean(boundary_violations_val**2)
                loss_val = loss_cri_val + loss_bound_val
